            ...
            <Model isVisible={showModal} onClose={() => setShowModal(false)} >
                <div className="mb-4 px-6 text-left">
                    ...
                    <form action="" className="space-y-6">
                        <div>
                            <label htmlFor="title" className={`block mb-2 text-sm font-medium ${mode === true ? 'text-black' : 'text-gray-600'}`}>Title</label>
                            <input type="text"
                                value={etitle}
                                onChange={(e) => setEtitle(e.target.value)}
                                name="etitle" id="etitle"
                                className={`${mode === true ? 'bg-[#2c2c2c]border-[2px] border-white text-white focus:ring-green-700 focus:border-blue-500' : 'bg-gray-300 border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500'}  outline-none  text-sm rounded-lg  block w-full p-2.5`} />
                        </div>
                        <div>
                            <label htmlFor="description" className={`block mb-2 text-sm font-medium ${mode === true ? 'text-black' : 'text-gray-600'}`}>Description</label>
                            <input type="text"
                                value={edescription}
                                onChange={(e) => setEdescription(e.target.value)}
                                name="edescription" id="edescription"
                                className={`${mode === true ? 'bg-[#2c2c2c]border-[2px] border-white text-white focus:ring-green-700 focus:border-blue-500' : 'bg-gray-300 border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500'} outline-none  text-sm rounded-lg  block w-full p-2.5`} />
                        </div>
                        <div>
                            <label htmlFor="tag" className={`block mb-2 text-sm font-medium ${mode === true ? 'text-black' : 'text-gray-600'}`}>Tag</label>
                            <input type="text"
                                value={etag}
